...
function isLastDay(){const now=new Date();const tomorrow=new Date(now.getTime()+86400000);return localToday().slice(0,7)!==iso(tomorrow).slice(0,7)}
function monthLabel(m){const[y,mo]=m.split('-').map(Number);return new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric',timeZone:TZ}).format(new Date(Date.UTC(y,mo-1,2)))}
...
function paymentEntries(s){
  if(Array.isArray(s.payment_history)&&s.payment_history.length)return s.payment_history.map(p=>({date:iso(p.date),amount:num(p.amount),method:payKind(p.method||s.payment_method)}));
  const d=iso(s.received_date||s.payment_received_date||s.updated_at||s.created_at),amt=num(s.paid_amount);
  return amt>0&&d?[{date:d,amount:amt,method:payKind(s.payment_method)}]:[];
}
...
function bar(label,value,max){const pct=max>0?Math.max(2,Math.round(value/max*100)):0;return `<div style="display:grid;grid-template-columns:130px 1fr 110px;gap:10px;align-items:center;margin:8px 0"><span style="font-size:12px;color:#52675e">${esc(label)}</span><div style="height:12px;border-radius:999px;background:#e8f0ec;overflow:hidden"><i style="display:block;height:100%;width:${pct}%;background:#0f684b"></i></div><b style="text-align:right;font-size:12px">${money(value)}</b></div>`}
...
if(markerSnap.exists&&!force){console.log(`Relatório ${month} já enviado.`);process.exit(0)}
...
let income=payments.reduce((a,p)=>a+p.amount,0);
let refunds=0;
sales.forEach(s=>{const r=num(s.refunded_amount);if(r>0&&monthOf(s.cancelled_at||s.updated_at)===month)refunds+=r});
income=Math.max(0,income-refunds);
const payBreak={pix:0,card:0,pix_installment:0,cash:0,transfer:0,other:0};payments.forEach(p=>payBreak[p.method]+=p.amount);
if(refunds>0)payBreak.other-=refunds;

const paidExp=expenses.filter(e=>expensePaid(e)&&monthOf(e.paid_date||e.expense_date||e.updated_at||e.created_at)===month);
const openExp=expenses.filter(e=>!expensePaid(e)&&monthOf(e.due_date||e.expense_date||e.created_at)===month);
const paidOut=paidExp.reduce((a,e)=>a+expenseAmount(e,seatsByTrip),0);
const toPay=openExp.reduce((a,e)=>a+expenseAmount(e,seatsByTrip),0);
const cashResult=income-paidOut;
...
const maxPay=Math.max(1,...Object.values(payBreak).map(v=>Math.abs(v)));
const byCat={};paidExp.forEach(e=>{const k=e.category||'Outros';byCat[k]=(byCat[k]||0)+expenseAmount(e,seatsByTrip)});
const maxCat=Math.max(1,...Object.values(byCat));
const rows=tripsMonth.map(x=>`<tr><td style="padding:10px;border-bottom:1px solid #e6ece9"><b>${esc(x.t.name||'Passeio')}</b><br><small>${esc(iso(x.t.trip_date))}</small></td><td style="padding:10px;text-align:center;border-bottom:1px solid #e6ece9">${x.seats}</td><td style="padding:10px;text-align:right;border-bottom:1px solid #e6ece9">${money(x.sold)}</td><td style="padding:10px;text-align:right;border-bottom:1px solid #e6ece9">${money(x.received)}</td><td style="padding:10px;text-align:right;border-bottom:1px solid #e6ece9">${money(x.cost.total)}</td><td style="padding:10px;text-align:right;border-bottom:1px solid #e6ece9">${money(x.actualPaid)}</td><td style="padding:10px;text-align:right;border-bottom:1px solid #e6ece9"><b>${money(x.profit)}</b></td><td style="padding:10px;text-align:center;border-bottom:1px solid #e6ece9">${x.breakEven??'—'}</td></tr>`).join('');

const html=`<div style="margin:0;padding:24px;background:#f1f6f3;font-family:Arial,Helvetica,sans-serif;color:#173a2f"><div style="max-width:980px;margin:auto;background:#fff;border-radius:22px;overflow:hidden;border:1px solid #dae7e1"><div style="padding:26px;background:#073226;color:#fff"><div style="font-size:12px;font-weight:800;letter-spacing:.08em">TRILHEIROS DE RONDONÓPOLIS</div><div style="font-size:27px;font-weight:900;margin-top:8px">Relatório financeiro mensal</div><div style="margin-top:5px;opacity:.85">${esc(monthLabel(month))}</div></div><div style="padding:24px"><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px"><div style="padding:16px;border-radius:14px;background:#eef6f2"><small>FATURADO NO MÊS</small><b style="display:block;font-size:22px;margin-top:5px">${money(booked)}</b></div><div style="padding:16px;border-radius:14px;background:#eef6f2"><small>DINHEIRO QUE ENTROU</small><b style="display:block;font-size:22px;margin-top:5px">${money(income)}</b></div><div style="padding:16px;border-radius:14px;background:#fff6df"><small>DESPESAS PAGAS</small><b style="display:block;font-size:22px;margin-top:5px">${money(paidOut)}</b></div><div style="padding:16px;border-radius:14px;background:#fff6df"><small>CONTAS A PAGAR</small><b style="display:block;font-size:22px;margin-top:5px">${money(toPay)}</b></div><div style="padding:16px;border-radius:14px;background:#edf8f2"><small>RESULTADO DE CAIXA</small><b style="display:block;font-size:22px;margin-top:5px">${money(cashResult)}</b></div><div style="padding:16px;border-radius:14px;background:#f6f7f6"><small>REEMBOLSOS NO MÊS</small><b style="display:block;font-size:22px;margin-top:5px">${money(refunds)}</b></div></div><h3 style="margin:28px 0 10px">Entradas por forma de pagamento</h3>${Object.entries(payBreak).filter(([,v])=>Math.abs(v)>0.009).map(([k,v])=>bar(PAY[k],v,maxPay)).join('')||'<p>Sem entradas confirmadas no período.</p>'}<h3 style="margin:28px 0 10px">Despesas pagas por categoria</h3>${Object.entries(byCat).map(([k,v])=>bar(k,v,maxCat)).join('')||'<p>Sem despesas pagas no período.</p>'}<h3 style="margin:28px 0 10px">Resultado por passeio</h3><div style="overflow:auto"><table style="border-collapse:collapse;width:100%;font-size:12px"><thead><tr style="background:#073226;color:#fff"><th style="padding:10px;text-align:left">Passeio</th><th>Clientes</th><th>Vendido</th><th>Recebido</th><th>Custo previsto</th><th>Desp. pagas</th><th>Lucro previsto</th><th>Equilíbrio</th></tr></thead><tbody>${rows||'<tr><td colspan="8" style="padding:16px">Nenhum passeio no mês.</td></tr>'}</tbody></table></div><p style="margin-top:22px;font-size:11px;color:#6b7d75">Critério: “dinheiro que entrou” usa recebimentos confirmados; despesas por pessoa em aberto usam a quantidade atual de clientes; quando a despesa é marcada paga, o valor fica congelado. O ponto de equilíbrio usa o preço médio real das reservas do passeio quando houver vendas.</p></div></div></div>`;
...
await marker.set({month,to:reportTo,sent_at:FieldValue.serverTimestamp(),summary:{booked,income,paid_out:paidOut,to_pay:toPay,cash_result:cashResult,refunds},forced:force},{merge:true});
console.log(`Relatório ${month} enviado para ${reportTo}.`);
