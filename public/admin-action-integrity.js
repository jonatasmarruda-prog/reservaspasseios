    ...
    q('#integrityPayForm',back).onsubmit=async e=>{
      ...
      try{
        ...
        await db.runTransaction(async tx=>{
          const ss=await tx.get(saleRef);if(!ss.exists)throw Error('Venda não encontrada.');const sd=ss.data()||{},saleTotal=n(sd.sale_total||sd.paid_amount),before=n(sd.paid_amount),newPaid=round(before+amount);if(newPaid>saleTotal+0.009)throw Error('O valor informado ultrapassa o total da venda.');
          ...
          tx.update(saleRef,patch);if(resRef){const rs=await tx.get(resRef);if(rs.exists)tx.update(resRef,{...patch,payment_history:[...((rs.data()||{}).payment_history||[]),entry]})}
        });
