...
let renderTimer=0,clockTimer=0;
...
function homeNow(){
  const now=new Date(),parts=new Intl.DateTimeFormat('pt-BR',{timeZone:TZ,weekday:'long',day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(now),get=t=>parts.find(x=>x.type===t)?.value||'';
  const hour=Number(get('hour')),greeting=hour<12?'Bom dia':hour<18?'Boa tarde':'Boa noite',weekday=get('weekday'),day=get('day'),month=get('month'),year=get('year'),minute=get('minute');
  return{greeting,dateText:`Hoje é ${weekday}, ${day} de ${month} de ${year}`,timeText:`${String(hour).padStart(2,'0')}:${minute}`};
}
function updateHomeClock(){
  clearTimeout(clockTimer);if(state.tab!=='dashboard')return;const box=q('#twHomeWeather');if(!box)return;
  const now=homeNow();setText(q('[data-tw-greeting]',box),`${now.greeting}, Jonatas 👋`);setText(q('[data-tw-datetime]',box),`${now.dateText} • ${now.timeText}`);
  clockTimer=setTimeout(updateHomeClock,30000);
}
...
function injectStyle(){
  ...
  .twWeatherHead{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.twWeatherHead h2,.twWeatherHead h3{margin:3px 0 4px;color:#073226}.twWeatherHead p{margin:0;color:#63786e;font-size:12px}.twGreeting{font-size:25px!important;line-height:1.15}.twDateTime{font-size:13px!important;font-weight:700;color:#50685e!important}.twLive{font-size:10px;font-weight:900;padding:6px 9px;border-radius:999px;background:#e6f6ed;color:#136746;white-space:nowrap}
  ...
  @media(max-width:780px){.twWeatherHead{align-items:flex-start}.twGreeting{font-size:21px!important}.twCurrent{grid-template-columns:1fr 1fr}.twTemp{grid-column:1/-1}.twTrip{grid-template-columns:1fr 1fr}.twTrip>div:first-child{grid-column:1/-1}.twTrip .twSoon{grid-column:1/-1}.v40ReportBar{grid-template-columns:1fr!important}}
...
async function mountHomeWeather(){
  if(state.tab!=='dashboard')return;const content=q('#content');if(!content||q('#twHomeWeather'))return;const now=homeNow();
  const box=document.createElement('section');box.id='twHomeWeather';box.className='twWeather';box.innerHTML=`<div class="twWeatherHead"><div><span class="eyebrow">VISÃO GERAL • RONDONÓPOLIS</span><h2 class="twGreeting" data-tw-greeting>${esc(now.greeting)}, Jonatas 👋</h2><p class="twDateTime" data-tw-datetime>${esc(now.dateText)} • ${esc(now.timeText)}</p></div><span class="twLive">AO VIVO</span></div><div class="twCurrent"><div><strong>Carregando previsão do tempo...</strong><small>Atualização automática pela internet.</small></div></div>`;content.insertBefore(box,content.firstChild);updateHomeClock();
  try{const d=await forecast(HOME.lat,HOME.lon,4);if(state.tab!=='dashboard'||!box.isConnected)return;const c=d.current||{},day=dailyAt(d,today()),currentNow=homeNow();box.innerHTML=`<div class="twWeatherHead"><div><span class="eyebrow">VISÃO GERAL • RONDONÓPOLIS</span><h2 class="twGreeting" data-tw-greeting>${esc(currentNow.greeting)}, Jonatas 👋</h2><p class="twDateTime" data-tw-datetime>${esc(currentNow.dateText)} • ${esc(currentNow.timeText)}</p></div><span class="twLive">AO VIVO</span></div><div class="twCurrent"><div class="twTemp">${Math.round(Number(c.temperature_2m||0))}°C</div><div><strong>${weatherLabel(c.weather_code)}</strong><small>Sensação ${Math.round(Number(c.apparent_temperature||c.temperature_2m||0))}°C • Vento ${Math.round(Number(c.wind_speed_10m||0))} km/h</small></div><div class="twMetric"><strong>${Math.round(Number(day?.max||0))}° / ${Math.round(Number(day?.min||0))}°</strong><small>Máx. / mín.</small></div><div class="twMetric"><strong>${Math.round(Number(day?.rain||0))}%</strong><small>Chance de chuva</small></div><div class="twMetric"><strong>${Number(c.precipitation||0).toFixed(1)} mm</strong><small>Precipitação agora</small></div></div>`;updateHomeClock()}catch(_){const currentNow=homeNow();box.innerHTML=`<div class="twWeatherHead"><div><span class="eyebrow">VISÃO GERAL • RONDONÓPOLIS</span><h2 class="twGreeting" data-tw-greeting>${esc(currentNow.greeting)}, Jonatas 👋</h2><p class="twDateTime" data-tw-datetime>${esc(currentNow.dateText)} • ${esc(currentNow.timeText)}</p></div><span class="twLive">AO VIVO</span></div><div class="twCurrent"><div><strong>Clima temporariamente indisponível</strong><small>Data e hora continuam atualizadas normalmente.</small></div></div>`;updateHomeClock()}
}
...
function schedule(delay=100){clearTimeout(renderTimer);if(state.tab!=='dashboard')clearTimeout(clockTimer);renderTimer=setTimeout(()=>{injectStyle();stabilizeCentralReports();repairOperationalReports();mountHomeWeather();mountTripsWeather()},delay)}
