...
(function(){
  ...
  let deferredPrompt=null,swRegistration=null,salesUnsub=null,salesBaseline=false;
  ...
  window.addEventListener('beforeinstallprompt',event=>{
    ...
    window.dispatchEvent(new CustomEvent('trilheiros-install-ready'));
  });
  ...
  async function mobileNotify({title='🥾 Trilheiros Gestão',body='',url='/admin',tag='trilheiros-gestao',force=false}={}){
    ...
    try{
      ...
      const n=new Notification(title,options);n.onclick=()=>{window.focus();location.href=url};return true;
    }catch(err){console.warn('Notificação:',err);return false}
  }
  window.trilheirosMobileNotify=mobileNotify;

  function saveHistory(){
    try{if(typeof state!=='undefined')localStorage.setItem(NOTIF_KEY,JSON.stringify((state.notifications||[]).slice(0,100)))}catch(_){ }
  }
  function refreshBadge(){
    try{
      if(typeof state==='undefined')return;
      const count=(state.notifications||[]).filter(x=>!x.read).length;
      const a=document.getElementById('notifyCount'),b=document.getElementById('sideNotifyCount');
      if(a)a.textContent=count?String(count):'';
  ...
  function addHistory({key,title,body,type='info',data={}},showSystem=false){
    ...
    state.notifications=[{id:(crypto?.randomUUID?.()||String(Date.now()+Math.random())),key,title,body,type,data,read:false,created_at:new Date().toISOString()},...state.notifications].slice(0,100);
    saveHistory();refreshBadge();
    if(showSystem)mobileNotify({title,body,url:data.url||'/admin',tag:key}).catch(()=>{});
  }
  ...
  async function requestNotifications(){
    ...
    try{
      ...
      if(permission==='granted'){
        ...
        try{toast('Permissão de notificações não concedida.','error')}catch(_){ }
      }
  ...
  function injectNotificationControls(){
    ...
    if(!content||document.getElementById('mobileNotificationStatus'))return;
    ...
    const box=document.createElement('div');box.id='mobileNotificationStatus';
    box.style.cssText='margin:16px 20px;padding:15px 16px;border-radius:16px;background:#edf7f2;border:1px solid #cfe5da;display:flex;gap:14px;justify-content:space-between;align-items:center;flex-wrap:wrap';
    ...
    const head=panel.querySelector('.panelHead');head?.after(box);
  ...
  function enhanceNotificationScreen(){
    const old=window.renderNotifications;
    if(typeof old!=='function'||old.__mobileEnhanced)return;
    const wrapped=function(...args){const out=old.apply(this,args);setTimeout(injectNotificationControls,0);return out};
    wrapped.__mobileEnhanced=true;window.renderNotifications=wrapped;
    try{globalThis.renderNotifications=wrapped}catch(_){ }
    window.requestAdminNotifications=requestNotifications;
  }
    ...
    try{
      ...
      salesUnsub=db.collection('sales').orderBy('created_at','desc').limit(200).onSnapshot(snapshot=>{
        ...
        snapshot.docChanges().forEach(change=>{
          const id=change.doc.id,s=change.doc.data(),next=saleSnapshotState(s),prev=saleState.get(id)||{paid:0,status:'',cancelled:false};
          saleState.set(id,next);
          if(change.type!=='modified')return;
          const delta=Math.max(0,next.paid-prev.paid);
          if(delta>0.009){
            const balance=Math.max(0,Number(s.balance_due||0));
            const title=`💰 Pagamento confirmado — ${s.customer_name||'Cliente'}`;
            const body=`${s.trip_name||'Passeio'} • ${paymentLabel(s.payment_method)} • ${money(delta)}${balance>0.009?` • saldo ${money(balance)}`:' • quitado'}`;
            const key=`payment:${id}:${next.paid.toFixed(2)}`;
  ...
  function applyRequestedTab(){
    ...
    const allowed=['dashboard','trips','people','finance','day','reports','pending','communications','feedback','notifications','settings','audit','trash','salesV21','safetyV40','waitlistV40'];
    if(tab&&allowed.includes(tab)&&state.tab!==tab){state.tab=tab;renderAdmin()}
  }

  window.addEventListener('load',()=>{
    enhanceNotificationScreen();
    setTimeout(()=>{enhanceNotificationScreen();applyRequestedTab()},350);
    try{
