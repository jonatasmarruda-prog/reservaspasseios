...
function b64ToUint8(v){const p='='.repeat((4-v.length%4)%4),s=(v+p).replace(/-/g,'+').replace(/_/g,'/'),raw=atob(s),out=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)out[i]=raw.charCodeAt(i);return out}
function tokenId(token){let h=0;for(let i=0;i<token.length;i++)h=((h<<5)-h)+token.charCodeAt(i)|0;return`owner_${Math.abs(h)}_${token.slice(-18).replace(/[^a-zA-Z0-9_-]/g,'')}`}
...
async function registerNativePush(force=false){
  if(nativeRunning||!location.pathname.startsWith('/admin'))return null;
...
async function registerPush(force=false){
  if(running||!location.pathname.startsWith('/admin'))return false;
