...
(function(){
  ...
  async function surveyPage(tripId){
    app.innerHTML=`<main class="surveyPage"><section class="surveyCard"><img src="https://trilheiros-reservas.web.app/assets/trilheiros-logo-email.png?v=20260925-hosted1" alt="Trilheiros de Rondonópolis"><span class="eyebrow">SUA EXPERIÊNCIA IMPORTA</span><h1>Como foi seu passeio?</h1><p class="surveyLead">Carregando informações...</p></section></main>`;
    try{
      ...
      if(old.exists){card.innerHTML=`<img src="https://trilheiros-reservas.web.app/assets/trilheiros-logo-email.png?v=20260925-hosted1" alt="Trilheiros"><div class="surveySuccess">✓</div><h1>Obrigado pela avaliação!</h1><p>Sua opinião sobre <b>${esc(t.name)}</b> já foi registrada.</p>`;return}
      card.innerHTML=`<img src="https://trilheiros-reservas.web.app/assets/trilheiros-logo-email.png?v=20260925-hosted1" alt="Trilheiros de Rondonópolis"><span class="eyebrow">SUA EXPERIÊNCIA IMPORTA</span><h1>${esc(t.name)}</h1><p class="surveyLead">Conta pra gente como foi. É rapidinho e ajuda a melhorar os próximos passeios.</p><form id="surveyForm"><div class="ratingGroup"><span>Sua nota</span><div class="ratingButtons">${[1,2,3,4,5].map(n=>`<button type="button" data-rating="${n}">${n}<small>★</small></button>`).join('')}</div></div><label><span>O que você mais gostou?</span><textarea name="liked" rows="3" placeholder="Pode contar pra gente..."></textarea></label><label><span>O que podemos melhorar?</span><textarea name="improve" rows="3" placeholder="Sua sugestão é bem-vinda"></textarea></label><label><span>Você indicaria os Trilheiros?</span><select name="recommend"><option value="sim">Sim, com certeza</option><option value="talvez">Talvez</option><option value="nao">Não</option></select></label><div id="surveyMsg"></div><button class="btn primary wide">Enviar avaliação</button></form>`;
      let rating=0;
      ...
      document.querySelector('#surveyForm').onsubmit=async e=>{e.preventDefault();if(!rating){document.querySelector('#surveyMsg').innerHTML='<div class="msg error">Escolha uma nota de 1 a 5.</div>';return}const f=e.target;await ref.set({rating,liked:f.liked.value.trim(),improve:f.improve.value.trim(),recommend:f.recommend.value,created_at:firebase.firestore.FieldValue.serverTimestamp()});card.innerHTML=`<img src="https://trilheiros-reservas.web.app/assets/trilheiros-logo-email.png?v=20260925-hosted1" alt="Trilheiros"><div class="surveySuccess">✓</div><h1>Valeu, trilheiro(a)! 💚</h1><p>Sua avaliação foi registrada. Nos vemos na próxima aventura!</p>`}
