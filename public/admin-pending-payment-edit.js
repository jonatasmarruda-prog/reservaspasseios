...
async function enhancePendingBalances(){
 ...
 for(let i=0;i<Math.min(rs.length,items.length);i++){
  ...
  const el=items[i],tag=el.querySelector('span'),strong=el.querySelector('strong');if(tag)tag.textContent=paid>0?(methodKind(method)==='pix_installment'?'PIX PARCELADO • PARCIAL':'PAGAMENTO PARCIAL'):'PAGAMENTO PENDENTE';if(strong)strong.textContent=`${name} • ${trip} • ${methodLabel(method)} • total ${money(total)} • recebido ${money(paid)} • falta ${money(balance)}`;el.title=paid>0?'Clique para editar e confirmar novos valores':'Clique para editar, confirmar ou excluir a pendência';el.onclick=e=>{e.preventDefault();window.openSyncedReservationV35(r.trip_id,r.id)};
 }
 const list=q('.pendingList');if(list&&rs.length&&!q('#ppeDebtBanner')){const box=document.createElement('div');box.id='ppeDebtBanner';box.className='ppeDebtBanner';box.innerHTML=`<b>💰 ${rs.length} pagamento(s) com saldo em aberto • ${money(totalOpen)}</b><small>${partialCount} já possuem entrada/parcela confirmada. Pendências sem nenhum pagamento podem ser editadas ou excluídas diretamente aqui.</small>`;list.parentElement?.insertBefore(box,list)}
}
