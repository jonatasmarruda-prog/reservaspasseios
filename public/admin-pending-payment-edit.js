/* Trilheiros Gestão — edição de pagamento diretamente nas pendências */
(function(){
'use strict';
...
window.openSyncedReservationV35=async function(tripId,id){
 ...
 const currentSuggested=method==='pix_installment'?round(Math.min(balance,n(sale?.next_installment_amount||r?.next_installment_amount)||balance)):balance;
 close();
 const back=document.createElement('div');back.id='pendingPaymentEditor';back.className='modalBack';
 back.innerHTML=`<div class="modal ppeModal"><div class="modalHead"><div><span class="eyebrow">CONFIRMAR / EDITAR PAGAMENTO</span><h2>${esc(name)}</h2><p>${esc(trip)} • ${esc(String(t?.trip_date||sale?.trip_date||'').slice(0,10))}</p></div><button type="button" class="iconClose" id="ppeClose">✕</button></div><div class="modalBody"><div class="ppeGrid"><div class="ppeCard"><span>VALOR TOTAL DO PASSEIO</span><strong>${money(total)}</strong></div><div class="ppeCard"><span>JÁ RECEBIDO</span><strong class="ppePaidValue">${money(paid)}</strong></div><div class="ppeField"><label for="ppeMethod">FORMA DE PAGAMENTO RECEBIDA</label><select id="ppeMethod"><option value="pix"${method==='pix'?' selected':''}>PIX</option><option value="card"${method==='card'?' selected':''}>Cartão de crédito</option><option value="pix_installment"${method==='pix_installment'?' selected':''}>PIX parcelado</option></select></div><div class="ppeField"><label for="ppeAmount">VALOR RECEBIDO AGORA</label><input id="ppeAmount" type="number" min="0.01" step="0.01" max="${balance.toFixed(2)}" value="${currentSuggested.toFixed(2)}" inputmode="decimal"></div><div class="ppeSummary"><div><span>TOTAL</span><strong>${money(total)}</strong></div><div class="ppePaid"><span>JÁ PAGO</span><strong id="ppeAlready">${money(paid)}</strong></div><div><span>CONFIRMANDO AGORA</span><strong id="ppeNow">${money(currentSuggested)}</strong></div><div class="ppeRemain"><span>RESTANTE</span><strong id="ppeRemaining">${money(Math.max(0,balance-currentSuggested))}</strong></div></div><div class="ppeNote" id="ppeNote">${method==='pix_installment'?'PIX parcelado: confirme somente o valor que realmente entrou. O saldo restante continuará aparecendo nas pendências e nos alertas antes do passeio.':'Você pode confirmar apenas um sinal/entrada. Se o valor for menor que o saldo total, a reserva ficará como pagamento parcial e continuará nas pendências.'}</div><div class="ppeCard ppeNames"><span>PARTICIPANTE(S)</span><strong>${esc(participantsOf(r,sale).join(' • ')||name)}</strong></div></div><div id="v35Msg"></div></div><div class="modalFoot"><button type="button" class="btn ghost" id="ppeCancel">Fechar</button>${balance>0.009?'<button type="button" class="btn primary" id="v35Confirm">Confirmar valor recebido</button>':'<button type="button" class="btn primary" disabled>Pagamento quitado</button>'}</div></div>`;
 document.body.appendChild(back);
 q('#ppeClose',back).onclick=q('#ppeCancel',back).onclick=close;back.onclick=e=>{if(e.target===back)close()};
 const amount=q('#ppeAmount',back),methodEl=q('#ppeMethod',back),nowEl=q('#ppeNow',back),remainEl=q('#ppeRemaining',back),note=q('#ppeNote',back),btn=q('#v35Confirm',back);
 const refresh=()=>{const v=Math.min(balance,n(amount?.value));if(nowEl)nowEl.textContent=money(v);if(remainEl)remainEl.textContent=money(Math.max(0,balance-v));if(note)note.textContent=methodEl?.value==='pix_installment'?'PIX parcelado: confirme somente o valor que realmente entrou. O saldo restante continuará aparecendo nas pendências e nos alertas antes do passeio.':'Você pode confirmar apenas um sinal/entrada. Se o valor for menor que o saldo total, a reserva ficará como pagamento parcial e continuará nas pendências.';if(btn)btn.textContent=v+0.009>=balance?'Confirmar e quitar':'Confirmar entrada '+money(v)};
 amount?.addEventListener('input',refresh);methodEl?.addEventListener('change',refresh);refresh();
...
window.confirmSyncedPaymentV35=async function(tripId,id,saleId){
 const btn=q('#v35Confirm'),msg=q('#v35Msg'),amountInput=q('#ppeAmount'),methodInput=q('#ppeMethod');
 if(btn){btn.disabled=true;btn.textContent='Confirmando...'}
 try{
  const requested=round(Number(amountInput?.value||0)),selectedMethod=methodKind(methodInput?.value||'pix'),resRef=db.collection('trips').doc(tripId).collection('reservations').doc(id),saleRef=db.collection('sales').doc(saleId||id),stamp=firebase.firestore.FieldValue.serverTimestamp(),date=day();
  ...
  await db.runTransaction(async tx=>{
   ...
   total=round(totalOf(rd,sd,t));const oldPaid=round(Math.max(n(rd.paid_amount),n(sd?.paid_amount))),remaining=round(Math.max(0,total-oldPaid));if(remaining<=0.009)throw Error('Pagamento já está quitado.');
   ...
   const common={sale_total:total,paid_amount:newPaid,balance_due:newBalance,payment_status:newStatus,payment_method:selectedMethod,received_date:date,last_payment_amount:confirmed,last_payment_method:selectedMethod,last_payment_confirmed_at:stamp,payment_alert_pending:newBalance>0.009,updated_at:stamp};
   tx.update(resRef,{...common,payment_history:[...(rd.payment_history||[]),entry],installment_total:selectedMethod==='pix_installment'?(rd.installment_total||sd?.installment_total||null):null});
   if(ss.exists)tx.update(saleRef,{...common,payment_history:[...(sd.payment_history||[]),entry],installment_total:selectedMethod==='pix_installment'?(sd.installment_total||rd.installment_total||null):null});
  });
  const local=(state.reservations||[]).find(x=>x.trip_id===tripId&&x.id===id);if(local){Object.assign(local,{sale_total:total,paid_amount:newPaid,balance_due:newBalance,payment_status:newStatus,payment_method:selectedMethod,received_date:date,last_payment_amount:confirmed});}
  close();
  notify(newStatus==='paid'?`${methodLabel(selectedMethod)} de ${money(confirmed)} confirmado. ${name} quitou ${tripName}.`:`${methodLabel(selectedMethod)} de ${money(confirmed)} confirmado. ${name} ainda tem ${money(newBalance)} para pagar em ${tripName}.`,'success');
  if(typeof renderAdmin==='function')renderAdmin();return{confirmed,newPaid,newBalance,newStatus,method:selectedMethod};
...
async function enhancePendingBalances(){
 ...
 for(let i=0;i<Math.min(rs.length,items.length);i++){
  ...
  const el=items[i],tag=el.querySelector('span'),strong=el.querySelector('strong');if(tag)tag.textContent=paid>0?(methodKind(method)==='pix_installment'?'PIX PARCELADO • PARCIAL':'PAGAMENTO PARCIAL'):'PAGAMENTO PENDENTE';if(strong)strong.textContent=`${name} • ${trip} • ${methodLabel(method)} • total ${money(total)} • recebido ${money(paid)} • falta ${money(balance)}`;el.title='Clique para editar a forma de pagamento e o valor recebido';el.onclick=e=>{e.preventDefault();window.openSyncedReservationV35(r.trip_id,r.id)};
 }
 const list=q('.pendingList');if(list&&rs.length&&!q('#ppeDebtBanner')){const box=document.createElement('div');box.id='ppeDebtBanner';box.className='ppeDebtBanner';box.innerHTML=`<b>💰 ${rs.length} pagamento(s) com saldo em aberto • ${money(totalOpen)}</b><small>${partialCount} já possuem entrada/parcela confirmada. Ao abrir uma pendência você pode editar o valor recebido e a forma de pagamento.</small>`;list.parentElement?.insertBefore(box,list)}
}
