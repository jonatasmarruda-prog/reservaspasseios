...
(function(){
  ...
  function revenueForTrip(trip){
    const rows=reservationsForTrip(trip.id),fromReservations=rows.reduce((sum,r)=>{
  ...
  function refreshTripFinancialPreview(){
    ...
    return (state.trips||[]).map(trip=>{
      ...
      trip.net_revenue=f.revenue;trip.expenses=f.expense;trip.profit=f.result;trip.estimated_cost=f.planned.total;trip.display_expenses_source=f.source;trip.display_planned_cost=f.planned.total;trip.display_paid_expenses=f.paid;trip.display_open_expenses=f.open;
      return f;
    });
  }
  function financeTotals(){
    const trips=refreshTripFinancialPreview(),revenue=trips.reduce((s,x)=>s+x.revenue,0),expenses=trips.reduce((s,x)=>s+x.expense,0),result=revenue-expenses;
  ...
  function patchDashboardMetrics(){
    ...
    document.querySelectorAll('.metric').forEach(card=>{
      ...
      if(label==='despesas'){strong.textContent=money(total.expenses);if(small)small.textContent='custos previstos/realizados'}
  ...
  function financeSelfTest(){
    ...
    const cases=[planned([{mode:'per_person',amount:50}],13)===650,planned([{mode:'fixed',amount:1000},{mode:'per_person',amount:50}],10)===1500,planned([{mode:'fixed',amount:300}],0)===300,expenseAmount({cost_mode:'per_person',amount:40,status:'pending'},5)===200,expenseAmount({cost_mode:'fixed',amount:400,status:'paid'},5)===400];
  ...
  function applyFinancialPreview(){refreshTripFinancialPreview();requestAnimationFrame(()=>{patchDashboardMetrics();patchV42TripResults();injectHealthPanel()})}
